import binascii
import sys
import struct
import gmpy2
from gmpy2 import mpz
from string import printable
from hashlib import md5

A = b'''

'''.replace(b'\n', b'')

a = int('''

'''.replace('\n', ''), 16)

B = int('''

'''.replace('\n', ''), 16)

b = int('''

'''.replace('\n', ''), 16) * a

K = hex(a).encode('ascii')
k = struct.unpack('<4L', K[:16])

S = list(range(256))
j = 0
for i in range(256):
    j = (j+S[i]+K[i%len(K)]) % 256
    S[i], S[j] = S[j], S[i]


def dec3(m):
    for i in range(0, len(m), 8):
        delta, mask = 0x9e3779b9, 0xffffffff
        sum = 0
        for round in range(32):
            sum = (sum+delta)&mask

        v0, v1 = struct.unpack('<2L', bytes(m[i:i+8]))
        for round in range(32):

            old_v1 = (v1 - ((sum + k[sum>>11&3])^ ((v0 << 4 ^ v0 >>5) + v0))) & mask
            sum = (sum-delta)&mask
            old_v0 =  (v0 - ((sum + k[sum&3])^ ((old_v1 << 4 ^ old_v1 >>5) + old_v1))) & mask
            v0, v1 = old_v0, old_v1
        e = struct.pack('<2L', old_v0, old_v1)

        for j in range(8):
            m[i+j] = e[j]
    return m


def dec2(m):
    j = 0
    for k in range(len(m)):
        i = (k+1)%256
        j = (j+S[i]) % 256
        S[i], S[j] = S[j], S[i]
        m[k] ^= S[(S[i]+S[j]) % 256]
    return m

def dec1(m):
    m = int(binascii.b2a_hex(bytes(m)), 16)
    p = gmpy2.invert(65537, (a-1)*(B-1))
    m = pow(m, p, b)
    return m

maps = {}
for ch in printable:
    maps[md5(ch.encode('ascii')).hexdigest()] = ch

def get_flag(m):
    flag = ''
    s = hex(m)[2:]
    for i in range(0, len(s), 32):
        v = s[i:i+32]
        flag += str(maps[v])
    return flag
#m = list(binascii.a2b_hex(sys.argv[1]))
m = list(binascii.a2b_hex(A))

m = dec3(m)
m = dec2(m)
m = dec1(m)

print(get_flag(m))

#print("output: ", binascii.b2a_hex(bytes(m)))
